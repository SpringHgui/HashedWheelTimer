...
namespace WheelTimer
{
    ...
    using WheelTimer.Concurrency;
    using WheelTimer.Internal;
    ...
    public sealed class HashedWheelTimer : ITimer
    {
            ...
            long maxPendingTimeouts)
        {
            ...
            tickDuration = tickInterval.Ticks;

            // Prevent overflow
            if (tickDuration >= long.MaxValue / wheel.Length)
            {
                throw new ArgumentException(
                    string.Format(
                        "tickInterval: {0} (expected: 0 < tickInterval in nanos < {1}",
                        tickInterval,
                        long.MaxValue / wheel.Length));
            }

            workerThread = new Thread(st => worker.Run());
        ...
        ~HashedWheelTimer()
        {
            ...
            if (Interlocked.Exchange(ref workerStateVolatile, WorkerStateShutdown) != WorkerStateShutdown)
            {
        ...
        bool ShouldLimitTimeouts => maxPendingTimeouts > 0;

        PreciseTimeSpan StartTime
        {
            get => PreciseTimeSpan.FromTicks(Volatile.Read(ref startTimeVolatile));
            set => Volatile.Write(ref startTimeVolatile, value.Ticks);
        }

        int WorkerState => Volatile.Read(ref workerStateVolatile);
        ...
        public void Start()
        {
            switch (WorkerState)
            {
                case WorkerStateInit:
                    if (Interlocked.CompareExchange(ref workerStateVolatile, WorkerStateStarted, WorkerStateInit) == WorkerStateInit)
                    {
                        workerThread.Start();
                    }
                    break;
            ...
            {
                startTimeInitialized.Wait(CancellationToken);
            }
        }
        ...
        public async Task<ISet<ITimeout>> StopAsync()
        {
            ...
            if (Interlocked.CompareExchange(ref workerStateVolatile, WorkerStateShutdown, WorkerStateStarted) != WorkerStateStarted)
            {
        ...
        public ITimeout NewTimeout(ITimerTask task, TimeSpan delay)
        {
            ...
            {
                long pendingTimeoutsCount = Interlocked.Increment(ref PendingTimeouts);
                if (pendingTimeoutsCount > maxPendingTimeouts)
                {
                    Interlocked.Decrement(ref PendingTimeouts);
        ...
        void ScheduleCancellation(HashedWheelTimeout timeout)
        {
            if (WorkerState != WorkerStateShutdown)
            {
                cancelledTimeouts.TryEnqueue(timeout);
            }
        }
        ...
        sealed class Worker : IRunnable
        {
            ...
            {
                this.owner = owner;
                closedPromise = new TaskCompletionSource();
            }

            public Task ClosedFuture => closedPromise.Task;
            ...
            public void Run()
            {
                try
                {
                    ...
                    owner.StartTime = PreciseTimeSpan.FromStart;
                    if (owner.StartTime == PreciseTimeSpan.Zero)
                    {
                        // We use 0 as an indicator for the uninitialized value here, so make sure it's not 0 when initialized.
                        owner.StartTime = PreciseTimeSpan.FromTicks(1);
                    }

                    // Notify the other threads waiting for the initialization at start().
                    owner.startTimeInitialized.Signal();

                    while (true)
                    {
                        TimeSpan deadline = WaitForNextTick();
                        if (Volatile.Read(ref owner.workerStateVolatile) != WorkerStateStarted)
                        {
                            break;
                        }
                        if (deadline > TimeSpan.Zero)
                        {
                            int idx = (int)(tick & owner.mask);
                            ProcessCancelledTasks();
                            HashedWheelBucket bucket = owner.wheel[idx];
                    ...
                    foreach (HashedWheelBucket bucket in owner.wheel)
                    {
                        bucket.ClearTimeouts(UnprocessedTimeouts);
                    }
                    while (owner.timeouts.TryDequeue(out var timeout))
                    {
                        if (!timeout.Canceled)
                        {
            ...
            void TransferTimeoutsToBuckets()
            {
                ...
                for (int i = 0; i < 100000; i++)
                {
                    ...
                    if (!owner.timeouts.TryDequeue(out timeout))
                    {
                        // all processed
                        break;
                    }
                    ...
                    long calculated = timeout.Deadline.Ticks / owner.tickDuration;
                    timeout.RemainingRounds = (calculated - tick) / owner.wheel.Length;

                    long ticks = Math.Max(calculated, tick); // Ensure we don't schedule for past.
                    int stopIndex = (int)(ticks & owner.mask);

                    HashedWheelBucket bucket = owner.wheel[stopIndex];
            ...
            void ProcessCancelledTasks()
            {
                ...
                {
                    HashedWheelTimeout timeout;
                    if (!owner.cancelledTimeouts.TryDequeue(out timeout))
                    {
                        // all processed
                        break;
                    }
            ...
            {
                long deadline = owner.tickDuration * (tick + 1);

                while (true)
                {
                    TimeSpan currentTime = (PreciseTimeSpan.FromStart - owner.StartTime).ToTimeSpan();
                    ...
                    try
                    {
                        ...
                        delay = Task.Delay((int)sleepTimeMs, owner.CancellationToken);
                        delay.Wait();
                    }
                    catch (AggregateException) when (delay != null && delay.IsCanceled)
                    {
                        if (Volatile.Read(ref owner.workerStateVolatile) == WorkerStateShutdown)
                        {
        ...
        sealed class HashedWheelTimeout : ITimeout
        {
            ...
            public bool Cancel()
            {
                ...
                timer.ScheduleCancellation(this);
                return true;
            }
            ...
            internal void Remove()
            {
                ...
                {
                    Interlocked.Decrement(ref timer.PendingTimeouts);
                }
            }
            ...
            {
                PreciseTimeSpan currentTime = PreciseTimeSpan.FromStart - timer.StartTime;
                TimeSpan remaining = Deadline - currentTime.ToTimeSpan();
